...
Although effective pressure is the common term to use, it is a bit inaccurate, and effective stress is more correct. In
the `fmu-pem` case we simplify the stress tensor to an isotropic pressure. To estimate effective pressure, we need to
...
provided by drilling or operation geology disciplines, and for most fields it will be a single one-dimensional model,
...
top reservoir, or a linear depth trend. Formation factor is set to 1.0. This is the settings in the YAML config file:

```yaml
# Overburden pressure may be set to a constant, but estimation is improved by using a depth trend
# Unit is Pa
pressure:
  type: trend
  intercept: 20.0e6
